...
const Card = ({ post }) => {
  ...
  return (
    ...
    <div className="card-img-overlay">
      ...
      <p className="position-absolute bottom-2 card-text text-white backdrop-blur-md">{post.body}</p>
      <div className="position-absolute bottom-0 end-0">
        {user.user_datum?.rol === "admin" && (
          <button type="button" className="btn btn-warning text-dark color-dark" data-bs-toggle="modal" data-bs-target="#exampleModal">
            Eliminar
          </button>
        )}
      </div>
    ...
    <div className="modal fade" id="exampleModal" tabIndex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div className="modal-dialog">
        <div className="modal-content">
          <div className="modal-header">
            <h1 className="modal-title fs-5 text-black" id="exampleModalLabel">Confirmación</h1>
            <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div className="modal-body">
            <p className="text-black">¿Estás seguro de que deseas eliminar este posteo?</p>
          </div>
          <div className="modal-footer">
            <button type="button" className="btn btn-dark text-dark" data-bs-dismiss="modal">Cancelar</button>
            <button type="button" className="btn btn-danger text-dark" onClick={handleDelete}>Eliminar</button>
