      ...
      <div className='row'>
        <nav className="navbar navbar-expand-lg navbar-dark bg-dark col-1 position-fixed vh-100 p-0 align-to mt-5 bg-transparent">
          <div className="navbar-nav flex-column h-200 d-flex ml-6 ">
            <a href='/home' className={`link ${selectedField === 'tech-news' ? 'active' : ''}`} onClick={() => setSelectedField('tech-news')}>
              <i className="bi bi-newspaper fs-4  text-success" ></i>
            </a>
    
            <a href='/community' className={`link ${selectedField === 'community' ? 'active' : ''}`} onClick={() => setSelectedField('community')}>
              <i className="bi bi-people fs-4 text-success"></i>
            </a>

            <a href='/JobsOffers' className={`link ${selectedField === 'job-applications' ? 'active' : ''}`} onClick={() => setSelectedField('job-applications')}>
